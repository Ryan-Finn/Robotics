...
def clear():
    global px, py, ang3, ang4, l1, l2, length, sliders, publisher
    px.set(length)
    py.set(0)
    ang3.set(0)
    ang4.set(0)
    ...
    a3 = l3.get()
    a4 = l4.get()
    sliders = bytes(str(length) + ':0:' + str(a1) + ':' + str(a2 + a3 + a4) + ':0:0:0', 'ascii')
...
def rand():
    ...
    t4 = 360 * np.random.rand() - 180
    # t3 = np.random.randint(-180, 181)
    # t4 = np.random.randint(-180, 181)
...
def update():
    global out, px, py, ang2, ang3, ang4, l1, l2, l3, l4, sliders, publisher
...
LabelInput4 = Tk.Label(CMD, text="t4: ")
LabelInput4.grid(row=4, column=1)
Entry4 = Tk.Entry(CMD, textvariable=ang4, width=10)

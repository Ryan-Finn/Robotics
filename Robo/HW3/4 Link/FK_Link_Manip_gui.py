...
def clear():
    ...
    servo4.config(from_=180, to=-180)
...
def update(foo):
    ...
    t3 = float(ang3.get())
    t4 = float(ang4.get())
    sliders = bytes(str(t1) + ':' + str(t2) + ':' + str(t3) + ':' + str(t4), 'ascii')
...
LabelServo4 = Tk.Label(Forward, text="t4")
...
servo4 = Tk.Scale(Forward, from_=180, to=-180, variable=ang4, command=update)
